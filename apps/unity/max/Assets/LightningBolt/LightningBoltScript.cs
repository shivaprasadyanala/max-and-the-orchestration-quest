...
namespace DigitalRuby.LightningBolt
{
    ...
    public class LightningBoltScript : MonoBehaviour
    {
		...
		public float randomEveryNSecond = 0f;
		public int randomToggleMinimum = 2;
		public int randomToggleMaximum = 4;
		public bool randomToggle = false;
		private bool randomToggleRepeated = true;
		private float timeElapsed = .1f;
		private int randomToggleNumber = 0;
        ...
        private void Update()
		{
			...
			if (randomToggle) {				
				if (timeElapsed >= randomToggleNumber) {
					if (randomToggleRepeated) {
						randomToggleRepeated = false;

						randomToggleNumber = UnityEngine.Random.Range (randomToggleMinimum, randomToggleMaximum);
					} else {
						randomToggleRepeated = true;
					}
					timeElapsed = 0f;
			...
			if (lineRenderer.enabled) {
				...
				if (Time.time % randomEveryNSecond < .3f)
					transform.rotation = Quaternion.Euler (0, 0, Random.Range (rotationRange.x, rotationRange.y));

				Debug.DrawLine (StartObject.transform.position, EndObject.transform.position, Color.green);
				RaycastHit2D hit = Physics2D.Linecast (StartObject.transform.position, EndObject.transform.position, CollisionLayer);
				if (hit) {
					//Debug.Log ("hit : " + hit.collider.gameObject.tag);
					if (explosion && !clone) {
						clone = Instantiate (explosion, hit.point, Quaternion.identity) as Explode1;
						clone.explosionPower = 10f;
					}
				}
